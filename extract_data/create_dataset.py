...
def create_dataset(images_folder: str, annotations_file: str, target_tfrecord: str) -> None:
    """Create a tfrecord of Animal Crossing villagers and save it.

    This function first creates a tensorflow dataset. After creation, a
    tfrecord file is saved in the specified path.

    Args:
        images_folder (str): Name of folder where images of villagers are
            stored.
        annotations_file (str): A JSON file with fields "name", "species",
            "personality", and "gender".
        target_tfrecord (str): File name of the saved tfrecord. Can be a full
            path.
    """
    ...
    for elem, villager in enumerate(annotations_json):
        if villager["name"] not in villager_names:
            ...
            villager["image"] = [
                os.path.join(images_folder, fn) for fn in filenames if villager["name"].replace(" ", "_") in fn
            ][0]
    annotations_json = [
        annotations_json[i] for i in range(len(annotations_json)) if i not in idx_to_remove
    ]
    ...
    for record in dataset:
        example = tf.train.Example(features=tf.train.Features(feature={
            "image": tf.train.Feature(bytes_list=tf.train.BytesList(value=[record["image"].numpy()])),
            "name": tf.train.Feature(bytes_list=tf.train.BytesList(value=[record["name"].numpy()])),
            "species": tf.train.Feature(bytes_list=tf.train.BytesList(value=[record["species"].numpy()])),
            "personality": tf.train.Feature(bytes_list=tf.train.BytesList(value=[record["personality"].numpy()])),
            "gender": tf.train.Feature(bytes_list=tf.train.BytesList(value=[record["gender"].numpy()])),
        }))
